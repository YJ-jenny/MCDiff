...
def get_wav(in_channels, pool=True):
    ...
    filter_LL = th.from_numpy(harr_wav_LL).unsqueeze(0)
...
class WaveUnpool(nn.Module):
    ...
    def forward(self, LL, LH, HL, HH, original=None):
        if self.option_unpool == 'sum':
            ...
            return th.cat([self.LL(LL), self.LH(LH), self.HL(HL), self.HH(HH), original], dim=1)
        else:
...
class UNetModifiedModel(nn.Module):
    """
    ...
    def __init__(
        ...
        self.wt = WavePool(int(ch//8))
        self.iwt = WaveUnpool(1)
        self.iwt_c = WaveUnpool(2)
        
        self.cond_enc = nn.Sequential(
            nn.Conv2d(in_channels-1, int(ch//8), 3, padding=1),
            nn.Conv2d(int(ch//8), int(ch//8), 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(int(ch//8), int(ch//8), 3, padding=1),
        ...
        self.input_conv = nn.Sequential(
            nn.Conv2d(in_channels-2, int(ch//8), 3, padding=1)
        )
        ...
        self.iwt_f = WaveUnpool(int(ch//4))
        self.features_extraction=TimestepEmbedSequential(
            ResBlock(
                int(ch//4),
                time_embed_dim,
                dropout,
        ...
        self.features_accumulation=TimestepEmbedSequential(
            ResBlock(
                128*2,
                time_embed_dim,
                dropout,
                out_channels=int(ch//4),
                dims=dims,
                use_checkpoint=use_checkpoint,
                use_scale_shift_norm=use_scale_shift_norm,
            )
        )
        self.wt_f = WavePool(int(ch//4))
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(dims, input_ch, (out_channels-2)*4, 3, padding=1)),
        )
        
        self.out2 = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(dims, input_ch, (out_channels-1)*4, 3, padding=1)),
        )
    ...
    def forward(self, x, timesteps, TR=None, TE=None, TI=None, y=None,latent=False,wt=False):
        ...
        ch_n = int(hs_tp.shape[1]//3)
        f_t1 = hs_tp[:,:ch_n].clone()
        f_t2 = hs_tp[:,ch_n:ch_n*2].clone()
        f_s0 = hs_tp[:,ch_n*2:].clone()
        

        t2_tr = TR[:,0:1]
        pd_tr = TR[:,1:2]
        t2_tr = t2_tr.unsqueeze(-1).unsqueeze(-1)
        pd_tr = pd_tr.unsqueeze(-1).unsqueeze(-1)
        ...
        for module in self.output_blocks2:
            h2 = th.cat([h2, hs2.pop()], dim=1)
            h2 = module(h2, emb)
        h2 = h2.type(x.dtype)
...
class SuperResModel(UNetModifiedModel):
    """
